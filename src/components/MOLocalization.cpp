...
//        RCLCPP_INFO(rclcpp::get_logger("localization"), "dx, dy : %f, %f",dx, dy);
        ...
        RCLCPP_INFO(rclcpp::get_logger("localization"), "(gps based) estimated initial pose - fx: %f, kfx: %f, dx: %f, fy: %f, kfy: %f, dy: %f", kfPose.x + rotatedDx, kfPose.x, rotatedDx, kfPose.y + rotatedDy, kfPose.y, rotatedDy);


        currentPose.x += (kfPose.x + rotatedDx)/pointSearchInd.size();
        currentPose.y += (kfPose.y + rotatedDy)/pointSearchInd.size();
        currentPose.z += kfPose.z/pointSearchInd.size();
    }
...
//
//
//        ndtResult.emplace_back(icp.getFitnessScore(), tCorrect);
//        //ndtResult.emplace_back(icp.getFitnessScore(), initialize_affine);
//    }
//
//    std::sort(ndtResult.begin(), ndtResult.end(),
//              [](const std::pair<float, Eigen::Affine3f>& a, const std::pair<float, Eigen::Affine3f>& b) {
...
//              });
//
//    auto [score, transform] = ndtResult[0];
//
//    for(auto [score2, transform2] : ndtResult){
//        std::cout << "ndt result" << score2 << std::endl;
//    }
//
//    float x, y, z, roll, pitch, yaw;
//    pcl::getTranslationAndEulerAngles(transform, x, y, z, roll, pitch, yaw);
...
//    initialize_pose[4] = y;
//    initialize_pose[5] = z;
//    RCLCPP_INFO(rclcpp::get_logger("localization"), "final initial pose : %f, %f, %f, %f, %f, %f, %f", x, y, z, roll, pitch, yaw, score);
//
//    has_initialize_pose = true;
}